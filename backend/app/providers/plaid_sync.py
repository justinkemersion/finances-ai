"""Plaid provider sync implementation"""
...
class PlaidSync(BaseProviderSync):
    """Plaid implementation of BaseProviderSync"""
    ...
    def sync_holdings(
        ...
        holdings_data = self.provider_client.get_investment_holdings(access_token)
        synced_holdings = []
    ...
    def sync_transactions(
        ...
        if sync_investment:
            try:
                investment_txns = self.provider_client.get_investment_transactions(
                    access_token,
        ...
        if sync_banking:
            try:
                ...
                if banking_account_ids:
                    banking_txns = self.provider_client.get_transactions(
                        access_token,
                        start_date.isoformat(),
                        end_date.isoformat(),
        ...
        return self._process_transactions(db, all_transactions_data)
    ...
    def _process_transactions(
        ...
        for txn_data in transactions_data:
            ...
            if transaction:
                self._update_transaction(transaction, txn_data, txn_date, txn_datetime, is_cancelled, classification)
            else:
                transaction = self._create_transaction(txn_data, txn_date, txn_datetime, is_cancelled, classification)
                db.add(transaction)
